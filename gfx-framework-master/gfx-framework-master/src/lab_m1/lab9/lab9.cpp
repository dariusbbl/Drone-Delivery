...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
		...
		// iau pozitia camerei si pozitia patratului
		glm::vec3 cameraPosition = GetSceneCamera()->m_transform->GetWorldPosition();
        glm::vec3 quadPosition = glm::vec3(0.0f, 0.5f, 0.0f);
		// calculez directia de la patrat la camera
        glm::vec3 directionToCamera = cameraPosition - quadPosition;
		// proiectez distanta pe planul OXZ
        directionToCamera.y = 0;
        float rotationAngle = atan2(directionToCamera.x, directionToCamera.z);
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    glUniform1i(glGetUniformLocation(shader->program, "mix_texture"), mixTextures);
    if (texture1)
    {
